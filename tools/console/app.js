...
function renderRuns(id) {
  $('runs').replaceChildren(...state.runs.map((run) => {
    ...
    button.title = `查看批次 ${run.id} · ${date(run.started_at)}`;
...
function renderItems() {
  ...
  for (const item of visible) {
    ...
    open.title = `查看 ${item.label} · ${fields[item.field] ?? (item.field || '整条记录')}`;
    ...
    const button = element('button', undefined, 'btn-ghost'); button.append(badge(status, !item.requiresDecision ? '查看依据' : historical() ? '查看' : statuses[status])); button.title = `查看 ${item.label} · ${button.textContent}`; button.addEventListener('click', () => showDetail(item)); action.append(button);
