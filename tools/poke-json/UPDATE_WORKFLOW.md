...
上游有新种族时，生成器会报错要求审核范围。当前支持全国编号 `1..1025` 和九世代，继续排除 Mega/Gmax 等答案，并保留形态合并、特性补充和静态分类规则。`has-mega` 先检查上游是否存在 Mega 形态，再应用已核实的形态限制：花叶蒂仅限 `floette-eternal`，基格尔德仅限 `zygarde-complete`；其他种族目前继续沿用种族检测并排除地区形态。依据和修复范围见 [Mega 形态校正](reports/2026-09-12-mega-forms.md)。进化优先使用上游 `is_default` 条件，没有该标记时保留原有顺序；项目的形态修正规则继续生效。
