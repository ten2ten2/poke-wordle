...
该批次已应用，Mega 修正保留于[后续发布数据](2026-09-12.md)。旧批次本地目录已清理；当前应用状态以[本地控制台](http://127.0.0.1:3318/)和 `src/data/dataset.json` 为准。该批次的 `mise run data:check` 已通过 Go 静态/竞态检查、形态生成回归与 10 项数据流程/控制台接口测试。
