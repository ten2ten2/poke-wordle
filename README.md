...
- `src/data` 保存离线游戏数据。[Go 数据工具](tools/poke-json/README.md) 位于 `tools/poke-json`，保留独立 Go 模块，与前端共用 Git 仓库和 mise 配置。输出到 `tools/poke-json/output`，审核后再复制到 `src/data`。
...
GitHub Actions 在 `dev`、`main` 推送及 Pull Request 时通过 mise 安装环境并执行 `mise run check`。数据工具测试使用本地模拟响应；联网生成数据需手动运行 `mise run data:run`。
