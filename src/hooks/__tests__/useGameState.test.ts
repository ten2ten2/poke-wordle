...
const mockPokemon: Pokemon[] = [
  {
    ...
    base_stats: { attack: 49, defense: 49, hp: 45, sp_attack: 65, sp_defense: 65, speed: 45 },
    evolution_stage: 1,
  ...
  },
  {
    ...
    base_stats: { attack: 62, defense: 63, hp: 60, sp_attack: 80, sp_defense: 80, speed: 60 },
    evolution_stage: 2,
    evolution_method: 'level',
...
describe('useGameState', () => {
  ...
  describe('Initialization', () => {
    test('initializes with default state', () => {
      ...
      expect(result.current.gameState.settings.selectedGenerations).toEqual([1, 2, 3, 4, 5, 6, 7, 8, 9]);
    ...
    test('restores game progress when available', () => {
      ...
      act(() => {
        // Trigger the effect that restores progress
      });
  ...
  describe('Settings Management', () => {
    ...
    test('updates selected generations and filters Pokemon', () => {
      ...
      mockedPokemonLib.filterPokemonByGenerations.mockReturnValue([mockPokemon[0], mockPokemon[1]]);
      
      act(() => {
        result.current.updateSettings({ selectedGenerations: newGenerations });
      });
      
      expect(result.current.gameState.settings.selectedGenerations).toEqual(newGenerations);
      expect(mockedPokemonLib.filterPokemonByGenerations).toHaveBeenCalledWith(mockPokemon, newGenerations);
    });
    ...
    test('resets game when target Pokemon is no longer available after generation change', () => {
      ...
      mockedPokemonLib.filterPokemonByGenerations.mockReturnValue([mockPokemon[2]]);
  ...
  describe('Guess Management', () => {
    test('adds guess correctly in reverse order', () => {
      ...
      const mockGuess = {
        ...
        base_stats_total: { value: 405, status: 'nope' as const, arrow: 'lower' as const },
  ...
  describe('Locale Changes', () => {
    test('updates Pokemon names when locale changes', () => {
      ...
      expect(result.current.pokemonNames).toEqual(['Bulbasaur', 'Ivysaur', 'Pikachu']);
      
      // Mock translation for Chinese
      mockedPokemonLib.translatePokemon.mockImplementation((pokemon, locale) => {
        if (locale === 'zh-hans') {
      ...
      expect(result.current.pokemonNames).toEqual(['Bulbasaur_CN', 'Ivysaur_CN', 'Pikachu_CN']);
    });
  });
}); 