...
describe('Settings', () => {
  ...
  describe('Guess Order Settings', () => {
    test('shows current guess order selection', async () => {
      ...
      expect(reverseButton).toHaveAttribute('aria-pressed', 'true'); // Selected
      expect(normalButton).toHaveAttribute('aria-pressed', 'false'); // Not selected
    });
    ...
    test('allows changing guess order', async () => {
      ...
      expect(normalButton).toHaveAttribute('aria-pressed', 'true'); // Now selected
    });
  ...
  describe('Max Guesses Settings', () => {
    test('shows current max guesses selection', async () => {
      ...
      expect(currentGuessButton).toHaveAttribute('aria-pressed', 'true');
    });
  ...
  describe('Generation Selection', () => {
    test('shows all generations as selected by default', async () => {
      ...
      for (let gen = 1; gen <= 9; gen++) {
        ...
        expect(genButton).toHaveAttribute('aria-pressed', 'true');
      }
    });
    ...
    test('allows toggling individual generations', async () => {
      ...
      expect(gen1Button).toHaveAttribute('aria-pressed', 'false'); // Deselected
    });
    ...
    test('deselect all button deselects all generations', async () => {
      ...
      for (let gen = 1; gen <= 9; gen++) {
        const genButton = screen.getByText(`Gen ${gen}`).closest('button');
        expect(genButton).toHaveAttribute('aria-pressed', 'false');
      }
    });
  });
  ...
  describe('Special Modes', () => {
    test('shows prankster mode toggle', async () => {
      ...
      expect(
        screen.getByText('Hide one random attribute each guess'),
      ).toBeInTheDocument();
    });
    ...
    test('shows generation arrow mode toggle', async () => {
      ...
      expect(
        screen.getByText('Show arrows for generation hints'),
      ).toBeInTheDocument();
    });
    ...
    test('allows toggling prankster mode', async () => {
      ...
      const pranksterToggle = screen.getByRole('checkbox', {
        name: 'Prankster Mode',
      });
    ...
    test('allows toggling generation arrow mode', async () => {
      ...
      const genArrowToggle = screen.getByRole('checkbox', {
        name: 'Generation Arrow Mode',
      });
  ...
  describe('Keyboard Navigation', () => {
    ...
    test('maintains settings state during user interaction', async () => {
      ...
      const normalOrderButton = screen
        .getByText('Oldest First')
        .closest('button');
