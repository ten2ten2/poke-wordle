...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const currentLocaleData =
    localeMetadata[locale as keyof typeof localeMetadata];

  // Generate correct URLs for English vs other locales
  const canonicalUrl = locale === 'en' ? '/' : `/${locale}`;
  const ogUrl =
    locale === 'en'
      ? 'https://www.pokewordle.app'
      : `https://www.pokewordle.app/${locale}`;
  ...
  return {
    ...
    openGraph: {
      ...
      alternateLocale: Object.values(localeMetadata)
        .map((data) => data.ogLocale)
        .filter((loc) => loc !== currentLocaleData.ogLocale),
    },
    twitter: {
